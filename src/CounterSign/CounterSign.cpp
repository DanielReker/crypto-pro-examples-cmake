...
/*
Пример создания удостоверяющей подписи, удовлетворяющей формату усовершенствованной подписи
формата CADES_X_LONG_TYPE_1 к уже существующей усовершенствованной подписи. Для работы примера 
первоначальная подпись должна быть сохранена в файл sign.dat (пример LowlevelSignCades). sign.dat 
должен находится в каталоге приложения. Для подписи необходимо чтобы в хранилище сертификатов 
присутствовал сертификат с закрытым ключом. Необходима работающая OCSP служба и ссылка на нее.
Удостоверяющая подпись будет сохранена в файл сountersign.dat
*/
...
int main(int argc, char *argv[]) {
    ...
    signPara.dwCadesType = CADES_X_LONG_TYPE_1; // Указываем тип усовершенствованной подписи
    ...
    // Создаём и вкладываем в сообщение удостоверяющую подпись
    ...
    // Сохраняем результат в файл countersign.dat
